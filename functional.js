function splitCSV(CSV){
    ...
    return (string) => {
        ...
        .split(' ')
        .map((word) =>{
            let fullCityname = prevWord+word
            if(cities[word]){ 
            ...
            }else if(cities[fullCityname]){
                return originalString
                .replace("назва міста", word)
                .replace('Х', cities[fullCityname].rating)
                .replace("УУУУУУ", cities[fullCityname].population)
...
console.log(getCity('Я приїхав з міста Біла Церква'));
